...
KEY_VALUE_RE = re.compile(r"(?P<key>[A-Za-z_][A-Za-z0-9_\-]*)=(?P<value>\"[^\"]*\"|'[^']*'|[^\s]+)")
COMPONENT_RE = re.compile(r"(?:^|\s)(?:component|service|module|app|source)=([^\s]+)", re.IGNORECASE)
SERVICE_NAME_RE = re.compile(r"^([a-zA-Z][a-zA-Z0-9_\-]*(?:\.[a-zA-Z][a-zA-Z0-9_\-]*)*)\s+")
...
def _extract_component(line: str, metadata: dict[str, Any]) -> tuple[Optional[str], str]:
	...
	# Try to extract service name from the start of the remaining line
	# after timestamp and severity have been removed (format: "service-name message")
	match = SERVICE_NAME_RE.match(line)
	if match:
		service_name = match.group(1)
		# Only treat it as a service if it looks like one (not a common word)
		if service_name.lower() not in ('the', 'and', 'for', 'with', 'from', 'this', 'that'):
			return service_name, line
...
def parse_log_line(line: str, line_number: int = 1) -> Optional[LogRecord]:
	...
	# Use component as service, or fall back to key_values
	service = component or key_values.get("service") or key_values.get("app") or key_values.get("module")
	message = remaining.strip() or raw_line.strip()
