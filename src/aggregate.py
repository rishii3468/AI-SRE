...
EVENT_CATEGORY_KEYWORDS: dict[str, tuple[str, ...]] = {
	"redis_timeout": ("redis timeout", "connection pool", "no available connections", "connection acquisition timeout"),
	"authentication_failure": ("authentication failed", "auth failure", "session validation timeout", "login failed"),
	"api_latency": ("latency", "slow response", "p95", "p99", "timeout"),
	"dns_failure": ("dns", "name resolution", "resolve host", "temporary failure in name resolution"),
	"oomkilled": ("oomkilled", "out of memory", "killed process", "memory cgroup"),
	"memory_leak": ("memory leak", "heap growth", "rss", "memory usage increased"),
	"disk_full": ("no space left on device", "disk full", "filesystem full"),
	"network_partition": ("network unreachable", "connection reset", "broken pipe", "network partition"),
	"load_balancer_failure": ("503", "502", "bad gateway", "load balancer"),
	"deployment_failure": ("deploy", "rollback", "release failed", "startup failed"),
	"dependency_outage": ("downstream", "dependency", "upstream", "service unavailable"),
	"message_queue_backlog": ("backlog", "queue depth", "consumer lag", "lagging"),
	"cache_stampede": ("cache miss", "stampede", "thundering herd"),
	"service_discovery_failure": ("service discovery", "discovery", "endpoint not found"),
	"rate_limit_exceeded": ("rate limit", "too many requests", "429"),
	"ssl_certificate_expired": ("certificate expired", "ssl", "tls handshake failed"),
	"mysql_lock_contention": ("lock wait timeout", "deadlock", "mysql lock"),
	"postgres_timeout": ("postgres", "query timeout", "statement timeout"),
}
